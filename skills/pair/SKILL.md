...
**2. Build — calibrate rigor, then decide inline vs subagent.** First set the *track* — it controls both the inner loop and the gate strategy (see `${SKILL_DIR}/../ruby-programming/references/calibrating-rigor.md`):
- **Discovery** (design emerging at the keyboard; no validated reference): strict per-change Red→Green→Refactor→Simplify→Pre-flight, full dual gates per phase. TDD earns its keep here.
- **Transcription** (design already validated — re-deriving a verified prototype, a mechanical port, a trivial change): batch the build, run the suite once or twice. The review gates still run every phase (see below) — Transcription streamlines the build, not the review. Per-change micro-TDD here is ceremony — the tests copy a known answer, and each micro-step is another spec boot and another full-context re-read. When unsure, default to Discovery. State the track in the phase plan and pass it to the builder.

Then decide inline vs subagent *before reading any build guidance*: if you offload, the builder reads it in its own context, so loading it here first wastes the savings. Default to inline; offer the subagent per phase, only when the user opts in, and favor it for mechanical phases over ones where the design is still being discovered at the keyboard.
...
**Quality + compliance gates — after every phase, both tracks.** Reviewing the code against our best practices is not optional in either track: Transcription streamlines the *build*, not the review. The prototype validated the design, not the port — ported code still has to clear our bar, and the prototype was never compliance-reviewed. On an unattended run the gates are the only safety net, so per-phase review matters more, not less. *How* to run them — always in parallel, since they're independent, don't wait for one before the other:
